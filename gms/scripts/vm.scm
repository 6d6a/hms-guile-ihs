...
(define (process-command command args opts)
  (for-each (lambda (arg)
              (let ((vds-account (vm->scm arg)))
                (case command
                  ((ip)
                   (let ((primary-ip-address (assoc-ref vds-account "primary_ip_address")))
                     ...
                     (newline)))
                  ((plan)
                   (let ((plan (assoc-ref vds-account "plan")))
                     (format #t "created: ~a~%" (assoc-ref plan "created"))
                     (format #t "name: ~a~%" (assoc-ref plan "name"))
                     (format #t "admin: ~a~%" (assoc-ref plan "adm"))
                     (newline)))
                  ((server)
                   (let* ((host (assoc-ref vds-account "host"))
                          (server (assoc-ref host "server")))
                     (format #t "name: ~a~%" (assoc-ref server "name"))
                     (newline)))
                  ((show)
                   (format #t "created: ~a~%" (assoc-ref vds-account "created"))
                   (format #t "changed: ~a~%" (assoc-ref vds-account "changed"))
                   (format #t "status: ~a~%" (assoc-ref vds-account "status"))
                   (format #t "state: ~a~%" (assoc-ref vds-account "state"))
                   (format #t "template_id: ~a~%" (assoc-ref vds-account "vds_template_id"))
                   (format #t "client_id: ~a~%" (assoc-ref vds-account "client_id"))
                   (format #t "vnc_port: ~a~%" (assoc-ref vds-account "vnc_port"))
                   (newline))
                  ((template)
                   (let ((template (assoc-ref vds-account "template")))
